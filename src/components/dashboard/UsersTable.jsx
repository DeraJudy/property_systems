...
export default function UsersTable() {
  ...
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [currentUserRole, setCurrentUserRole] = useState(null); // Track the viewer's role
  ...
  // Permission check: only admin and hr can see actions/assign roles
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold foreground-text">Users</h1>
          <p className="text-sm muted-foreground-text">Manage system users and permissions</p>
        </div>
        <Button>
                ...
                <TableRow key={user.id} className="hover:bg-[#e6f2ec]/30">
                  <TableCell>
                    <div className="flex items-center py-2 px-2 gap-3">
                      ...
                      <span className="font-medium foreground-text">{user.full_name}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${roleColor(user.role)}`}>
                      {user.role}
                    </span>
                  </TableCell>
