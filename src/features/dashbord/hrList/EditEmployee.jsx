...
const EditEmployee = () => {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        if (error) throw error;
        if (data) {
          // Ensure all arrays are initialized to prevent .map crashes
          setFormData({
            ...data,
            personal_records: data.personal_records || [],
            staff_documents: data.staff_documents || [],
            references_attachments: data.references_attachments || [],
            training_induction_records: data.training_induction_records || [],
  ...
  const handleFileUpload = async (e, fieldName, index = null, listName = null) => {
  ...
  const triggerDeleteConfirm = (field, index) => {
    ...
    setItemToDelete({ field, index, name: item.name || "this document", url: item.url });
  };
  ...
  const handleSubmit = async () => {
    ...
    status.completedCount < status.total ? setIsConfirmOpen(true) : processUpdate();
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6 min-h-screen">
      ...
      {[
        ...
        { id: "references_attachments", title: "Reference Document", icon: FileCheck },
        { id: "training_induction_records", title: "Training & Induction", icon: BookOpen },
      ...
      <AlertDialog open={!!itemToDelete} onOpenChange={() => !isDeleting && setItemToDelete(null)}>
        <AlertDialogContent className="bg-[#f7f2e9]">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Document?</AlertDialogTitle>
            <AlertDialogDescription>Permanent deletion of <strong>{itemToDelete?.name}</strong>.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={(e) => { e.preventDefault(); handleConfirmDelete(); }} className="bg-red-600 text-white" disabled={isDeleting}>
      ...
      <AlertDialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <AlertDialogContent className="bg-[#f7f2e9]">
          <AlertDialogHeader>
            <AlertDialogTitle>Save Incomplete Profile?</AlertDialogTitle>
            <AlertDialogDescription>Some sections are empty. Continue?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
