...
function DeleteConfirmationModal({ isOpen, onClose, onConfirm, itemName }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white border-red-100">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-600 font-medium">
            This will permanently remove <span className="font-bold text-black italic">"{itemName}"</span> and all associated legal documents, images, and certificates from the storage bucket. This action cannot be undone.
          </DialogDescription>
        </DialogHeader>

        <div className="py-4 space-y-3">
          <Label className="text-[10px] font-black uppercase tracking-widest opacity-60">
            Type <span className="text-red-600 underline">DELETE</span> to confirm
          </Label>
          <Input
            value={confirmationText}
            onChange={(e) => setConfirmationText(e.target.value)}
            placeholder="DELETE"
            className="border-red-200 focus:ring-red-500 h-12 text-center font-bold text-red-600"
          />
        </div>

        <DialogFooter className="gap-2">
          <Button variant="ghost" onClick={onClose} disabled={isDeleting} className="font-bold">
            Cancel
          </Button>
          <Button
            variant="destructive"
            ...
            className="bg-red-600 hover:bg-red-700 px-8 font-bold uppercase tracking-widest"
          >
            {isDeleting ? <Loader2 className="animate-spin mr-2" /> : "Permanently Purge"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default function PropertiesTable() {
  ...
  const handleDelete = async () => {
    if (!propertyToDelete) return;
    const toastId = toast.loading(`Purging ${propertyToDelete.property_name}...`);

    try {
      // 1. Fetch all certificates to get their storage paths
      const { data: certs } = await supabase
        .from("property_certificates")
        .select("document_url")
        .eq("property_id", propertyToDelete.id);

      // 2. Identify all paths in the bucket
      const pathsToDelete = [];
      if (propertyToDelete.image_url) pathsToDelete.push(propertyToDelete.image_url);
      if (propertyToDelete.lease_url) pathsToDelete.push(propertyToDelete.lease_url);
      if (propertyToDelete.insurance_url) pathsToDelete.push(propertyToDelete.insurance_url);
      
      certs?.forEach(cert => {
        if (cert.document_url) pathsToDelete.push(cert.document_url);
      });
      ...
      if (pathsToDelete.length > 0) {
        ...
        if (storageError) console.error("Bucket cleanup failed:", storageError);
      }

      // 4. Delete Database Record (Certificates will cascade delete if set in DB)
      ...
      toast.success("Property and all assets deleted successfully", { id: toastId });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header / Stats Section (Keeping your existing UI style) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* ... (Existing Stats Cards) */}
        
      </div>

      <Card className="border-[#e1dbd2] bg-[#FFFDD0] backdrop-blur-sm shadow-xl overflow-hidden rounded-3xl">
        <div className="p-6 border-b border-black/5 bg-black/5 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="relative w-full md:w-96">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
            <Input
              placeholder="Search properties..."
              className="pl-10 bg-white border-black/10 rounded-xl"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <Button 
              onClick={() => router.push("/addProperties")}
              className="bg-black hover:bg-black/90 text-[#FFFDD0] rounded-xl px-6 font-bold"
            >
          ...
          <Table>
            <TableHeader className="bg-black/5">
              <TableRow className="hover:bg-transparent border-black/5">
                <TableHead className="font-black text-[10px] uppercase tracking-widest py-6">Property</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest">Location</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest">Org</TableHead>
                  ...
                  >
                    <TableCell className="py-5">
                      <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-xl bg-gray-100 overflow-hidden border border-black/5 shadow-inner">
                          {property.image_url ? (
                            <img 
                            ...
                            <Home className="h-full w-full p-3 text-gray-300" />
                          )}
                        </div>
                        <div>
                          <p className="font-bold text-black">{property.property_name}</p>
                          <p className="text-xs text-gray-400 font-medium">Rooms: {property.rooms}</p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <span className="text-xs font-bold bg-black/5 px-3 py-1 rounded-full uppercase tracking-tighter italic">
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-white rounded-xl shadow-2xl border-black/5">
                          <DropdownMenuItem onClick={() => router.push(`/properties/${property.id}`)} className="font-bold gap-2 cursor-pointer">
                            <Eye className="w-4 h-4" /> View
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => router.push(`/properties/${property.id}/edit`)} className="font-bold gap-2 cursor-pointer">
                          ...
                          <DropdownMenuItem 
                            ...
                            className="font-bold gap-2 text-red-600 cursor-pointer focus:bg-red-50 focus:text-red-700"
                          >
      ...
      <DeleteConfirmationModal
        ...
        itemName={propertyToDelete?.property_name}
