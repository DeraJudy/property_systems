...
export default function ViewServiceUser() {
  ...
  // --- IMMEDIATE UPLOAD FUNCTION (Handles bad network) ---
  const handleImmediateUpload = async (file, type) => {
    ...
    try {
      ...
      const filePath = `${id}/kws_${Date.now()}.${fileExt}`;
      ...
      toast.success(`${type === "doc" ? "Document" : "Evidence"} uploaded.`);
    } catch (error) {
  ...
  //   try {
  //     // 2. REQUIRED FIELD FIX (this is the whole bug)
  ...
  //       // ✅ REQUIRED COLUMN (fixes your error)
  //       file_url: fileUrl,
...
const handleEditClick = (doc) => {
  ...
  const LiveThumbnail = ({ doc }) => {
    ...
    const ext =
      doc.original_name?.toLowerCase().split(".").pop() ||
  ...
  return (
    ...
    >
                    ...
                    <DialogContent className="sm:max-w-[450px]">
                      <DialogHeader>
                        <DialogTitle className="text-[#123d2b] font-black flex gap-2">
                          <Briefcase className="w-5 h-5" /> ADD SESSION RECORD
                        </DialogTitle>
                      </DialogHeader>
                      ...
                      <div className="space-y-5 py-4">
                        <div className="space-y-1">
                          ...
                          <Input
                            placeholder="e.g. Tenancy Support Session"
                            value={kwsName}
                        ...
                        {/* Document Field */}
                        <div
                          className={`p-4 rounded-xl border-2 border-dashed ${docUrl ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-100"}`}
                        >
                          <label className="text-[10px] font-black text-slate-500 block mb-2">
                            1. KEY WORKING DOCUMENT
                          </label>
                        ...
                        {/* Media Field */}
                        <div
                          className={`p-4 rounded-xl border-2 border-dashed ${mediaUrl ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-100"}`}
                        >
                          <label className="text-[10px] font-black text-slate-500 block mb-2">
                            2. VIDEO/IMAGE EVIDENCE
                          </label>
                          <div className="flex items-center gap-2">
                            <input
                              type="file"
                        ...
                        <Button
                          className="w-full bg-[#123d2b] h-12"
                          disabled={
                            !docUrl ||
                            !mediaUrl ||
                            !kwsName ||
                    ...
                    day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit'
                        ...
                        <td className="p-4">
                          <Button
                            ...
                            onClick={() =>
                              doc.doc_url && openDocument(doc.doc_url)
                            }
                          >
                        ...
                        <td className="p-4">
                          <div
                            ...
                            onClick={() =>
                              doc.media_url && setViewingMedia(doc.media_url)
                            }
                          >
                            {doc.media_url?.match(/\.(mp4|webm|mov)/) ? (
                        ...
                        <td className="p-4 text-right flex gap-2 justify-end">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-8 w-8 text-red-600 bg-red-50 hover:bg-red-100 border border-red-100"
                            onClick={() => {
                              setDocToDelete(doc); // Set the document to delete
     ...
     {/* UPLOAD/EDIT MODAL */}
      <Dialog open={isUploadModalOpen} onOpenChange={(open) => !open && closeModal()}>
        <DialogContent className="sm:max-w-[450px]">
          <DialogHeader>
        ...
        <DialogContent className="max-w-4xl">
          ...
          {viewingMedia && (
            <div className="w-full">
              {viewingMedia.match(/\.(mp4|webm|mov|ogg)$/) ? (
                <video
                  src={viewingMedia}
                  controls
                  className="w-full rounded-lg"
                />
              ) : viewingMedia.match(/\.(jpg|jpeg|png|gif|webp)$/) ? (
                <img src={viewingMedia} className="w-full rounded-lg" />
              ) : (
                <iframe src={viewingMedia} className="w-full h-[500px]" />
              )}
            </div>
          )}
        </DialogContent>
        ...
        <DialogContent className="bg-white border-[#e1dbd2]">
          ...
          <div className="space-y-4 py-4">
            <p className="text-sm text-[#123d2b]">
              ...
              <span className="font-bold">"{docToDelete?.file_name}"</span>.
